...
const ProductInfo = ({
  ...
  return (
    <div className="flex flex-col justify-center lg:items-start text-right lg:order-1">
      ...
      {/* DEBUG: DEEP SCAN MODE */}
      <div className="text-xs text-left bg-gray-900 text-green-400 p-4 mb-4 dir-ltr font-mono overflow-auto border-4 border-red-500 rounded-lg shadow-2xl relative z-50">
        <h3 className="font-bold text-white border-b border-gray-700 pb-2 mb-2">
          🕵️ STORIA PRICE DETECTIVE
        </h3>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <strong className="text-white">Main Product:</strong>
            <br />
            Price: {product.rawPrice}
            <br />
            Regular: {product.rawRegularPrice}
            <br />
            Sale: {product.rawSalePrice}
            <br />
          </div>
          <div>
            <strong className="text-white">Logic Check:</strong>
            <br />
            IsOnSale: {product.isOnSale ? "YES" : "NO"}
            <br />
            Calc Reg: {product.regularPrice}
            <br />
            Calc Sale: {product.salePrice}
            <br />
          </div>
        </div>

        <div className="mt-4 border-t border-gray-700 pt-2">
          <strong className="text-white">Looking for "580":</strong>
          <br />
          {JSON.stringify(product.rawSallaData).includes("580") ? (
            <span className="text-xl bg-green-600 text-white px-2 font-bold animate-pulse">
              FOUND "580" IN RAW DATA!
            </span>
          ) : (
            <span className="text-xl bg-red-600 text-white px-2 font-bold">
              "580" NOT FOUND IN MAIN DATA
            </span>
          )}
        </div>

        <div className="mt-4 border-t border-gray-700 pt-2">
          <strong className="text-white">First Variant Check:</strong>
          <br />
          {product.sizeVariants && product.sizeVariants.length > 0 ? (
            <pre className="text-[10px] whitespace-pre-wrap text-yellow-300">
              {JSON.stringify(product.sizeVariants[0], null, 2)}
            </pre>
          ) : (
            <span className="text-red-400">No variants found</span>
          )}
        </div>

        <div className="mt-4 border-t border-gray-700 pt-2">
          <strong className="text-white">Full Raw Data Dump (Start):</strong>
          <br />
          <pre className="text-[9px] text-gray-400 whitespace-pre-wrap h-32 overflow-y-auto">
            {JSON.stringify(product.rawSallaData, null, 2)}
          </pre>
        </div>
      </div>
