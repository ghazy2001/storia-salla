/* STORIA DESIGN LOADER v15.1 - HYPER SPEED MODE */
(function () {
  const path = window.location.pathname.toLowerCase();
  const VERCEL_URL = "https://storia-salla.vercel.app";
  const TIMESTAMP = Date.now();

  console.log("🚀 Storia Design Loader v15.1: Hyper Speed Mode...");
  ...
  style.innerHTML = `
    ...
    #root { display: block; width: 100%; height: 100%; transition: opacity 0.5s ease; }
    #native-preloader {
  ...
  const init = () => {
    ...
    if (isSallaPage) {
      ...
      const hideName = () => {
        ...
        document.querySelectorAll("h1, h2, h3, span, p").forEach((el) => {
    ...
    // HYPER FAST REMOVAL: Just 300ms overlap to cover raw mounting
      ...
      if (document.documentElement.classList.contains("storia-ready")) {
        ...
        setTimeout(() => {
          ...
          if (lp) {
            ...
            setTimeout(() => lp.remove(), 500);
          }
        }, 300); // Only 300ms delay instead of 1200ms
      }
    }, 50);
